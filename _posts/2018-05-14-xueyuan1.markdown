...
## 正文
早段时间报名了百度前端技术学院的第三个学院，设计师学院。今天抽空把第一个任务做出来啦，这个demo主要是运用到了CSS transition。感兴趣的同学可以访问[设计师学院任务（一）]（http://ife.baidu.com/course/detail/id/18）进行查看。下面我把今天写的代码贴出来，再进行一些总结与分享。  
